...
function startAutoRefresh() {
    if (SyncManager.autoRefreshInterval) {
        clearInterval(SyncManager.autoRefreshInterval);
    }
    
    SyncManager.autoRefreshInterval = setInterval(function() {
        if (!SyncManager.isSyncing && !SyncManager.isLoadingFromCloud && !SyncManager.isProcessingQueue) {
            console.log('Auto-refresh: checking for updates from spreadsheet...');
            loadFromSpreadsheet(true);
        }
    }, SyncManager.AUTO_REFRESH_MS);
    
    console.log('Auto-refresh started (every ' + (SyncManager.AUTO_REFRESH_MS / 1000) + ' seconds)');
}

function stopAutoRefresh() {
    if (SyncManager.autoRefreshInterval) {
        clearInterval(SyncManager.autoRefreshInterval);
        SyncManager.autoRefreshInterval = null;
 ...
 * @param {boolean} silent - If true, skip loading overlay (for background refresh)
...
function loadFromSpreadsheet(silent) {
    ...
    return fetch(url)
        ...
        .then(function(result) {
            if (silent) {
                console.log('Silent refresh: data received');
            } else {
...
function reloadBeforeAction(callback) {
    if (!SyncManager.enabled || !SyncManager.url) {
        if (typeof callback === 'function') callback();
        return;
    }
    
    loadFromSpreadsheet(true).then(function() {
        if (typeof callback === 'function') callback();
    });
}
...
function initVisibilityListener() {
    document.addEventListener('visibilitychange', function() {
        if (document.visibilityState === 'visible' && SyncManager.enabled) {
            var timeSinceLastSync = Date.now() - (SyncManager.lastSyncTime || 0);
            var MIN_SYNC_INTERVAL = 5000;
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initVisibilityListener);
} else {
