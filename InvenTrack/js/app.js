...
function initSearchAndFilters() {
    try {
        // Search Peminjaman
        const searchPeminjaman = document.getElementById('searchPeminjaman');
        if (searchPeminjaman) {
            searchPeminjaman.addEventListener('input', function (e) {
                try {
                    filterTableRows('peminjamanTable', e.target.value.toLowerCase());
                } catch (error) {
                    console.error('❌ Search peminjaman error:', error);
                }
            });
        }
        ...
        // Search Kerusakan
        const searchKerusakan = document.getElementById('searchKerusakan');
        if (searchKerusakan) {
            searchKerusakan.addEventListener('input', function (e) {
                try {
                    filterTableRows('kerusakanTable', e.target.value.toLowerCase());
                } catch (error) {
                    console.error('❌ Search kerusakan error:', error);
                }
            });
        }
        ...
        if (searchRiwayat) {
            searchRiwayat.addEventListener('input', function (e) {
                try {
                    filterTableRows('riwayatTable', e.target.value.toLowerCase());
                } catch (error) {
                    console.error('❌ Search riwayat error:', error);
                }
            });
        }
                ...
                <tbody>
                    ${kerusakan.map((k, idx) => `
                        <tr>
                            <td>${idx + 1}</td>
                            <td>${formatDate(k.createdAt)}</td>
                            <td>${k.laptopKode} - ${k.laptopNama}</td>
                            <td>${k.dilaporkanOleh}</td>
...
function exportKerusakan() {
    ...
    let csv = 'No,Tanggal,Kode Laptop,Nama Laptop,Dilaporkan Oleh,Jenis Kerusakan,Kategori,Deskripsi,Status\\n';

    kerusakan.forEach((k, index) => {
        csv += `${index + 1},"${formatDate(k.createdAt)}","${k.laptopKode}","${k.laptopNama}","${k.dilaporkanOleh}","${k.jenisKerusakan}","${k.kategori || ''}","${k.deskripsi || ''}","${k.status}"\\n`;
    });

    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'laporan_kerusakan_' + new Date().toISOString().split('T')[0] + '.csv';
    link.click();

    showToast('Data kerusakan berhasil diexport!');
}
...
// ==========================================
// HELPER FUNCTION: FILTER TABLE ROWS
// ==========================================
